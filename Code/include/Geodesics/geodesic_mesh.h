...
inline void Mesh::build_adjacencies()
{
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)
		{
			...
			half_edges[k].vertex_0 = std::min<unsigned>(vertex_id_1, vertex_id_2);
			half_edges[k].vertex_1 = std::max<unsigned>(vertex_id_1, vertex_id_2);
...
inline bool Mesh::verify()		//verifies connectivity of the mesh and prints some debug info
{
	...
	for(unsigned i=0; i<m_edges.size(); ++i)
	{
		...
		longest_edge = std::max<double>(longest_edge, e.length());
		shortest_edge = std::min<double>(shortest_edge, e.length());
	}
	...
	for(unsigned i=0; i<m_faces.size(); ++i)
	{
		...
		for(unsigned j=0; j<3; ++j)
		{
			...
			min_angle = std::min<double>(min_angle, angle);
			max_angle = std::max<double>(max_angle, angle);
		}
	}
