...
inline long GeodesicAlgorithmExact::visible_from_source(SurfacePoint& point)	//negative if not visible
{
	...
	if(point.type() == EDGE)		
	{
		edge_pointer e = static_cast<edge_pointer>(point.base_element());
		list_pointer list = interval_list(e);
		double position = std::min<double>(point.distance(e->v0()), e->length());
...
{
	if(p->min_func() < GEODESIC_INF/10.0)// && p->min >= queue->begin()->first)
	{
															   ...
															   IntervalWithStop* one)			//intersecting two intervals with up to three intervals in the end
{
	...
	assert(one->min_func() < GEODESIC_INF/10.0);

	double const local_epsilon = SMALLEST_INTERVAL_RATIO*one->edge()->length(); 

	unsigned N=0;
	if(zero->min_func() > GEODESIC_INF/10.0)
	{
		start[0] = zero->start();
	...
	double left = std::max<double>(zero->start(), one->start());		//define left and right boundaries of the intersection of the intervals
	double right = std::min<double>(zero->stop(), one->stop());
									   ...
									   std::vector<SurfacePoint>* stop_points)
{
	...
	m_propagation_distance_stopped = m_queue.empty() ? GEODESIC_INF : (*m_queue.begin())->min_func();
	clock_t stop = clock();
...
{
	double queue_distance = (*m_queue.begin())->min_func();
																	...
																	interval_pointer source_interval)	//up to two candidates
{
	...
	if(num_candidates == 2)		
	{
		double start = std::min<double>(candidates->start(), (candidates + 1)->start());
		double stop = std::max<double>(candidates->stop(), (candidates + 1)->stop());
	...
	if(!invert)					//in this case everything is straighforward, we do not have to invert the intervals
	{
		for(unsigned i=0; i<num_candidates; ++i)
		{
			...
			p->min_func() = 0.0;					//it will be changed later on
...
{					
	...
	double best_total_distance; 
	double best_interval_position;
	unsigned source_index = UINT_MAX;
