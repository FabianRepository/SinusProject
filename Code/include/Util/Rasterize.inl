...
void Rasterize(const std::vector< Point3D<float> >& vertices, const std::vector< TriangleIndex >& triangles, CubeGrid< char >& grid, float scale, int threads)
{
	if (!grid.resolution())
	{
...
#pragma omp parallel for num_threads( threads )
	for (int i = 0; i<triangles.size(); i++)
		Rasterize(vertices[triangles[i][0]] * float(grid.resolution()), vertices[triangles[i][1]] * float(grid.resolution()), vertices[triangles[i][2]] * float(grid.resolution()), grid, scale);
}

void RasterizeEdges(const std::vector< Point3D<float> >& vertices, const std::vector< TriangleIndex >& triangles, CubeGrid< char >& grid, float scale, int threads)
{
	if (!grid.resolution())
	{
...
#pragma omp parallel for num_threads( threads )
	for (int i = 0; i<triangles.size(); i++) for (int j = 0; j<3; j++)
		Rasterize(vertices[triangles[i][j]] * float(grid.resolution()), vertices[triangles[i][(j + 1) % 3]] * float(grid.resolution()), grid, scale);
}

void Rasterize(Point3D<float> v1, Point3D<float> v2, Point3D<float> v3, CubeGrid< char >& grid, float scale)
{
	if (!grid.resolution())
	{
	...
	Point3D<float> w1 = v2 - v1, w2 = v3 - v1;
	float l1 = Point3D<float>::SquareNorm(w1), l2 = Point3D<float>::SquareNorm(w2);
	if (!l1 && !l2) // The triangle is a point
		Rasterize(v1, grid, scale);
	else if (!l1) // The triangle is the line from v1 to v3
		Rasterize(v1, v3, grid, scale);
	else if (!l2) // The triangle is the line from v1 to v2
		Rasterize(v1, v2, grid, scale);
	else
	{
		w1 /= float(sqrt(double(l1)));
		float dot = Point3D<float>::Dot(w1,w2);
		float l = float(sqrt(l2 - dot*dot));
		int steps = (int)(l + 3);
		steps = (int)(steps*scale);
		for (int i = 0; i<steps; i++)
		{
			float t = float(i) / (steps - 1);
			Rasterize(v2*(1 - t) + v1*t, v2*(1 - t) + v3*t, grid, scale);
		}
	}
}

void Rasterize(Point3D<float> v1, Point3D<float> v2, CubeGrid< char >& grid, float scale)
{
	Point3D<float> w = v2 - v1;
	float l = float(sqrt(double(Point3D<float>::SquareNorm(w))));
	int steps = (int)(l + 3);
	steps = (int)(steps*scale);
	for (int i = 0; i<steps; i++)
	{
		float t = float(i) / (steps - 1);
		Rasterize(v1*t + v2*(1 - t), grid);
	}
}

void Rasterize(Point3D<float> v, CubeGrid< char >& grid, float scale)
{
